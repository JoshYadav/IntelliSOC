...
const pieColors = ['#06b6d4', '#3b82f6', '#818cf8', '#f43f5e', '#fb923c', '#10b981'];
const getAttackTypeColor = (type: string) => {
  ...
  return pieColors[Math.abs(hash) % pieColors.length];
};
  ...
  if (active && payload?.length) return (
    <div style={{
      ...
      {payload.map((p: any, i: number) => (
        <p key={i} style={{ color: '#06b6d4', fontWeight: 600 }}>{p.value}</p>
      ))}
    </div>
  );
  return null;
